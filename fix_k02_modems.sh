#!/bin/bash
# K02 모뎀 복구 및 라우팅 설정 전용 스크립트 (V1.2 - Dynamic Ready)
...
# K02의 경우 enp10s0이 메인입니다.
PRIMARY_IFACE="enp10s0"
sudo nmcli connection modify $(sudo nmcli -t -f NAME,DEVICE connection show active 2>/dev/null | grep ":${PRIMARY_IFACE}$" | cut -d: -f1) ipv4.route-metric 50 2>/dev/null || true
sudo nmcli connection up $(sudo nmcli -t -f NAME,DEVICE connection show active 2>/dev/null | grep ":${PRIMARY_IFACE}$" | cut -d: -f1) 2>/dev/null || true
