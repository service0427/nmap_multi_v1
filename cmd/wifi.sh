...
if [ -n "$SELECTED_DEVICES_ARG" ]; then
    ...
    if [ ${#all_devices[@]} -gt 1 ]; then
        ...
        # Determine dynamic default device numbers based on chosen SSID (e.g. -11 -> 1-15, -12 -> 16-30)
        default_devs="0"
        if [[ "$chosen_ssid" =~ -([0-9]+)$ ]]; then
            suffix_num=${BASH_REMATCH[1]}
            if [ "$suffix_num" -ge 11 ]; then
                idx_offset=$((suffix_num - 11))
                start_dev=$((idx_offset * 15 + 1))
                end_dev=$(( (idx_offset + 1) * 15 ))
                default_devs="${start_dev}-${end_dev}"
            fi
        fi

        read -p "Select device number(s) (e.g., 0, 1-15, 1,3,9-12) [Default: $default_devs]: " dev_selection < /dev/tty
